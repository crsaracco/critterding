...
ScGraphics::ScGraphics( std::shared_ptr<BeGraphicsSystem> graphicsSystem, BeFilesystem& filesystem )
...
// 	m_hdr_effect = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/default.vert", "shaders/default.frag"));
...
// 					m_crittervision_effect.reset(new BeGraphicsEffect(m_filesystem, "shaders/crittervision_new.vert", "shaders/crittervision_new.frag"));
	...
	m_luminanceEffect2 = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/simple.vert", "shaders/luminance2x2.frag"));
	m_luminanceEffect3 = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/simple.vert", "shaders/luminance2x2.frag"));
	m_brightEffect = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/simple.vert", "shaders/brightpass.frag"));
	m_gaussianBlurHorizonalEffect = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/simple.vert", "shaders/gaussianH.frag"));
	m_gaussianBlurVerticalEffect = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/simple.vert", "shaders/gaussianV.frag"));
	m_tonemapEffect = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/simple.vert", "shaders/tonemap.frag"));
	m_addRgbEffect = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/simple.vert", "shaders/addrgb.frag"));
...
// 	m_sceneTexture = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, width, height));
// 	m_sceneRenderbuffer = std::shared_ptr<BeRenderbuffer>(new BeRenderbuffer(GL_DEPTH_COMPONENT, width, height));
// 	m_sceneFramebuffer = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
//
...
// 	m_luminanceTexture2 = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, width, height));
// // 	m_luminanceTexture2 = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, LUM2TEXTURE_W, LUM2TEXTURE_H));
// 	m_luminanceFramebuffer2 = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
...
// 		m_luminanceFramebuffer3[i] = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
// 		m_luminanceTexture3[i] = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, size, size));
...
// // 	m_brightTexture = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, width/4, height/4));
// // 	m_brightTexture = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, width/2, height/2));
// 	m_brightTexture = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, width, height));
// 	m_brightFramebuffer = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
// 	m_brightFramebuffer->texture2D(GL_COLOR_ATTACHMENT0, m_brightTexture->handle(), 0);


// 	m_tonemapTexture = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, width, height));
// // 	m_tonemapTexture = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, TONETEXTURE_W, TONETEXTURE_H));
// 	m_tonemapFramebuffer = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
// 	m_tonemapFramebuffer->texture2D(GL_COLOR_ATTACHMENT0, m_tonemapTexture->handle(), 0);

// 	for(int i=0;i<2;++i)
// 	{
//
// // 		m_gaussianTexture[i]=std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, width/4, height/4));
// 		m_gaussianTexture[i]=std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, width, height));
// 		m_gaussianFramebuffer[i] = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
// 		m_gaussianFramebuffer[i]->texture2D(GL_COLOR_ATTACHMENT0, m_gaussianTexture[i]->handle(), 0);
// 	}

	//DOF

	m_dofSceneEffect = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/default_dof.vert", "shaders/default_dof.frag"));
	m_dofSkyboxEffect = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/skybox_dof.vert", "shaders/skybox_dof.frag"));

	m_dofDownsampleEffect = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/second.vert", "shaders/second.frag"));
	m_dofFilterXEffect = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/third.vert", "shaders/third.frag"));
	m_dofFilterYEffect = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/fourth.vert", "shaders/fourth.frag"));
	m_dofFinalEffect = std::shared_ptr<BeGraphicsEffect>(new BeGraphicsEffect(filesystem, "shaders/fifth.vert", "shaders/fifth.frag"));

// 	m_dofSceneTexture = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, width, height));
// 	m_dofSceneFramebuffer = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
// 	m_dofSceneFramebuffer->texture2D(GL_COLOR_ATTACHMENT0, m_dofSceneTexture->handle(), 0);
//
// 	for(int i=0;i<3;++i)
// 	{
// 		m_dofDownsampleTexture[i] = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, width, height));
// 		m_dofDownsampleFramebuffer[i] = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
...
void ScGraphics::resetBrightTexture( const int w, const int h )
{
	if ( width_prev != w || height_prev != h )
	{
		...
		m_dofSceneTexture = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, w, h));
		m_dofSceneFramebuffer = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
		m_dofSceneFramebuffer->texture2D(GL_COLOR_ATTACHMENT0, m_dofSceneTexture->handle(), 0);
		...
		for(int i=0;i<3;++i)
		{
			...
			m_dofDownsampleTexture[i] = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, w, h));
			m_dofDownsampleFramebuffer[i] = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
			m_dofDownsampleFramebuffer[i]->texture2D(GL_COLOR_ATTACHMENT0, m_dofDownsampleTexture[i]->handle(), 0);
			m_dofDownsampleTexture[i]->parameter(GL_TEXTURE_WRAP_S, GL_CLAMP);
			m_dofDownsampleTexture[i]->parameter(GL_TEXTURE_WRAP_T, GL_CLAMP);
		...
		m_sceneTexture = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, w, h));
		m_sceneRenderbuffer = std::shared_ptr<BeRenderbuffer>(new BeRenderbuffer(GL_DEPTH_COMPONENT, w, h));
		m_sceneFramebuffer = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
		...
		m_brightTexture = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, w, h));
		m_brightFramebuffer = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
		m_brightFramebuffer->texture2D(GL_COLOR_ATTACHMENT0, m_brightTexture->handle(), 0);
	...
	// 	m_tonemapTexture = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, width, height));
		...
		m_tonemapTexture = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, w, h));
		m_tonemapFramebuffer = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
		m_tonemapFramebuffer->texture2D(GL_COLOR_ATTACHMENT0, m_tonemapTexture->handle(), 0);
	...
	// 		m_gaussianTexture[i]=std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, width/4, height/4));
			...
			m_gaussianTexture[i]=std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, w, h));
			m_gaussianFramebuffer[i] = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
			m_gaussianFramebuffer[i]->texture2D(GL_COLOR_ATTACHMENT0, m_gaussianTexture[i]->handle(), 0);
		}

		m_luminanceTexture2.reset();
		m_luminanceTexture2 = std::shared_ptr<BeTexture2D>(new BeTexture2D(GL_RGBA16F, w, h));
		m_luminanceFramebuffer2 = std::shared_ptr<BeFramebuffer>(new BeFramebuffer());
		m_luminanceFramebuffer2->texture2D(GL_COLOR_ATTACHMENT0, m_luminanceTexture2->handle(), 0);
		m_luminanceTexture2->parameter(GL_TEXTURE_WRAP_S, GL_CLAMP);
		m_luminanceTexture2->parameter(GL_TEXTURE_WRAP_T, GL_CLAMP);
