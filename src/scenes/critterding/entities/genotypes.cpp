...
// 	gt->speciescolor = colorH.randomBeColorRGB().getNormalized();
	gt->speciescolor = colorH.randomBeColorRGB();
...
Genotype* Genotypes::loadGenotype(std::string& content)
{
// 	cerr << "loading genotype start" << endl;
	Genotype* gt = new Genotype();

	std::string passToBody;
	std::string passToBrain;
	contentParser.reset();
	...
	while ( !line.empty() )
	{
		parseH.reset();
		// adamdist=690;
			if ( parseH.beginMatchesStrip( "adamdist=", line ) )
			{
				std::string AD = parseH.returnUntillStrip( ';', line );
				//cerr << "AD: " << AD << endl;
		...
		// the rest goes to the body or brain
			else
			{
				if (
					...
					parseH.beginMatches( "m ", line ) ||
					parseH.beginMatches( "c ", line ) ||
					parseH.beginMatches( "cm ", line )
				)
				{
