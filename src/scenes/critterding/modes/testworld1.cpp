...
void TestWorld1::init()
{
	...
	for ( unsigned int i=0; i < elevations; i++ )
	{
		...
		Wall* w = new Wall( (float)settings->getCVar("worldsizeX")/(decSizeFactor*(i+2)), WallWidth, (float)settings->getCVar("worldsizeY")/(decSizeFactor*(i+2)), t, m_dynamicsWorld );
// 		w->color.r = 0.34f+(0.005f*i); w->color.g = 0.25f+(0.005f*i); w->color.b = 0.11f+(0.005f*i);
		w->color.setR(0.34f+(0.005f*i));
		w->color.setG(0.25f+(0.005f*i));
		w->color.setB(0.11f+(0.005f*i));
		entities.push_back(w);
	}
}

void TestWorld1::process(const BeTimer& timer)
{
	// recalc freeEnergy
		calcFreeEnergy();
